...
const create = async (req: Request, res: Response): Promise<void> => {
  ...
  const createdPost = result as PostEntity;
  res.status(201).json(Post.toResponse(createdPost));
};

const update = async (req: Request<IdParams>, res: Response): Promise<void> => {
  const { title, text, userId } = req.body || {};
  const patch: { title?: string; text?: string; userId?: string } = {};
  if (title !== undefined) patch.title = title;
  if (text !== undefined) patch.text = text;
  if (userId !== undefined) patch.userId = userId;
  const post = await postsService.update(req.params.id, patch);
  if (post && 'error' in post && post.error === 'USER_NOT_FOUND') {
    res.status(404).json({ message: 'User not found' });
    return;
  }
  if (!post) {
